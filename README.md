...
shared route-document renderer
      ├──────── request time ────────► Hono HTTP response
      │
      └──────── build time ──────────► dist/static/**/index.html
                                      + dehydrated Query/Cache state
      ↓
browser Query/Cache + DOM bindings + form/action dataflows
      ↓
Bun runtime/build adapter
```
...
The older `/hello/:name` code remains in `src/examples/routes.tsx` as a typed-routing proof for path-derived params and `href()` generation; it is not part of the generated M06–M09 application route tree.
