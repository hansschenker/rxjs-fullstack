...
The current home-page SSR pipeline demonstrates this directly: `defer()` creates a cold request dataflow, the resolved model is mapped through `HomePage`, then through `renderToString`, and finally through `renderDocument`.
...
Hono owns HTTP routing. The current application exposes:

- `GET /health` → `{ "ok": true }`,
- `GET /` → the SSR-generated RxJS Fullstack HTML document.

The root route consumes the cold `homeDocument$` dataflow with `firstValueFrom()`. This is the explicit boundary where Hono's async request handler asks the RxJS dataflow for the response value. The route does not move rendering logic into Hono; it only bridges the completed RxJS result into `context.html(...)`.

The server application and runtime adapter are intentionally separate:

- `src/server/app.tsx` defines the Web-API Hono application and the RxJS-backed routes.
- `src/server/bun.ts` is only the Bun adapter: it supplies port `3000` and forwards Bun's `fetch` handling to `app.fetch`.

That split means Bun is not framework semantics. A later Node, Deno, Cloudflare, or other Web-API-compatible adapter can host the same Hono application without changing the JSX runtime, RxJS execution model, or HTML renderer.
...
The first slice therefore demonstrates the project's central rule: **RxJS is the application machine; the surrounding technologies keep their existing jobs.**
