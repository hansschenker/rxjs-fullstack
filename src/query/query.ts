...
/**
 * The contract a query result stream (`query$` subscription) fulfils towards
 * its `Query` — the RxJS equivalent of TanStack's `QueryObserver` link.
 */
export interface QueryObserverLike {
  ...
  getCurrentResult: () => { isStale: boolean };
}
...
> extends Removable {
  ...
  #cache: QueryCache;
  #client: QueryClient;
  #retryer?: Retryer<TData> | undefined;
  observers: Array<QueryObserverLike>;
  #defaultOptions?:
  ...
  constructor(config: QueryConfig<TQueryFnData, TError, TData, TQueryKey>) {
    ...
    this.#cache = this.#client.queryCache;
  ...
  isActive(): boolean {
    return this.observers.some(
      (observer) =>
        resolveQueryBoolean(observer.options.enabled, this as any) !== false,
    );
  }
  ...
  isStatic(): boolean {
    if (this.getObserversCount() > 0) {
      return this.observers.some(
        (observer) =>
          resolveStaleTime(observer.options.staleTime, this as any) ===
          "static",
      );
    }
  ...
  async fetch(
    ...
    if (
      ...
      if (this.state.data !== undefined && fetchOptions?.cancelRefetch) {
        ...
        // make sure that retries that were potentially cancelled due to unsubscribes can continue
    ...
    if (!this.options.queryFn) {
      ...
      if (observer) {
        this.setOptions(observer.options as any);
      }
    }
    ...
    const behavior =
      this.#queryType === "infinite"
        ? (infiniteQueryBehavior(
          ...
          ) as unknown as QueryBehavior<TQueryFnData, TError, TData, TQueryKey>)
    ...
    const retryer = (this.#retryer = createRetryer<TData, TError>({
    ...
    try {
      ...
      if (error instanceof CancelledError) {
        if (error.silent) {
          ...
          // to reject with a CancelledError
          if (this.state.data === undefined) {
            throw error;
          }
  ...
  #dispatch(action: Action<TData, TError>): void {
    ...
    this.#cache.notify({ query: this as any, type: "updated", action });
  }
}
