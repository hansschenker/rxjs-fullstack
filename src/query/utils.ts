...
// Copied from TanStack Query, itself copied from jonschlinkert/is-plain-object
...
/** `placeholderData` helper that shows the previous query's data while a new key loads. */
...
>(
  ...
  // when we don't have a queryFn yet, we can't retry, so we just return the
  // already rejected initialPromise
...
/**
 * Adds a lazily-consumed `signal` property to `object`; `onCancelled` fires
 * once when a consumed signal aborts.
 */
